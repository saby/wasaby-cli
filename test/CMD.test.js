...
describe('CMD', () => {
   ...
   describe('execute', () => {
      ...
      test('should throw error if stdout contents error label', (done) => {
         stubExec = jest.spyOn(child_process, 'exec').mockImplementation(() => {
            const process = getProcess();
         ...
         cmd.execute('help', undefined, { errorLabel: '[error]:' }).catch((errors) => {
            expect(errors[0]).toStrictEqual('[error]: ttttt');
            done();
         });
      });
