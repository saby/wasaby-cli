...
describe('Repository', () => {
   ...
   describe('detectCheckoutStrategy()', () => {
      ...
      test('should delete branch and return strategy with checkout and merge if branch is not rc', async() => {
         ...
         expect(stubBranch.mock.calls[0][1]).toStrictEqual('-D');
      });
      ...
      test('should checkout rc if current branch is required', async() => {
         ...
         expect(stubCheckout.mock.calls[0][1]).toStrictEqual('-f');
      });
   ...
   describe('delete()', () => {
      ...
      beforeEach(() => {
         ...
         stubExecute = jest.spyOn(repository._cmd, 'execute').mockReturnValue(Promise.resolve());
      });
      ...
      test('should delete repository on window platform', async() => {
         ...
         expect(stubExecute.mock.calls[0][0]).toStrictEqual('DEL /F/Q/S Rep > NUL');
         expect(stubExecute.mock.calls[0][1]).toStrictEqual('src/dir');
         expect(stubExecute.mock.calls[0][2]).toEqual({
            processName: 'Rep delete'
         });

         expect(stubExecute.mock.calls[1][0]).toStrictEqual('RMDIR /Q/S Rep');
         expect(stubExecute.mock.calls[1][1]).toStrictEqual('src/dir');
         expect(stubExecute.mock.calls[1][2]).toEqual({
            processName: 'Rep delete'
         });
      });
      ...
      test('should delete repository on linux platform', async() => {
         ...
         expect(stubExecute.mock.calls[0][0]).toStrictEqual('sudo rm -rf Rep');
         expect(stubExecute.mock.calls[0][1]).toStrictEqual('src/dir');
         expect(stubExecute.mock.calls[0][2]).toEqual({
            processName: 'Rep delete'
         });
      ...
      test('should return exception if could not delete repository', async() => {
         stubExecute.mockImplementation(() => {
            throw 'error delete';
         });
         ...
         try {
            ...
            expect(err).toStrictEqual('error delete');
         }
      });
   });
   ...
   describe('update()', () => {
      ...
      test('should reset, clean and fetch repository', async() => {
         ...
         expect(stubReset.mock.calls[0][0]).toStrictEqual('--hard');

         expect(stubClean.mock.calls.length).toStrictEqual(1);
         expect(stubClean.mock.calls[0][0]).toStrictEqual('-fdx');

         expect(stubFetch.mock.calls.length).toStrictEqual(1);
         expect(stubFetch.mock.calls[0][0]).toStrictEqual('--all --prune');
      });
   ...
   describe('getChangedFiles()', () => {
      ...
      test('should call git diff with only name and status between revision and HEAD', async() => {
         ...
         expect(stubDiff.mock.calls[0][2]).toStrictEqual('--name-status');
      });
