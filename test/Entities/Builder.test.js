...
const defaultBuilderConfig = {
   ...
   cache: 'build/cache',
   version: '24.3000-1',
   nodeModules: pathUtils.join(process.cwd(), 'node_modules'),
   symlinks: true,
   useReact: true,
   tsc: true,
   mode: 'stand',
   ...
   moduleType: 'amd',
   regionPlaceholdersPath: 'src/logs/regionPlaceholdersPath.json',
};
...
function buildOptions(options = {}) {
   ...
   result.set('artifactsDir', 'artifactDir/artifactDir');
   result.set('rc', 'rc-24.3000');
...
describe('Builder', () => {
   ...
   describe('constructor()', () => {
      test('should create builder with debug config', () => {
         const builder = new Builder(new Map(), buildOptions());
      ...
      test('should create builder with tsconfig path', () => {
         ...
         const builder = new Builder(new Map(), buildOptions({}), tsconfigPath);
   ...
   describe('_buildModulesListForConfig()', () => {
      ...
      function expectModulesConfig(config, expected = {}) {
         ...
         if (expectedConfig.featuresProvided) {
            expectedConfig.featuresProvided = expectedConfig.featuresProvided.map((feature) => feature[1]);
         }

         if (expectedConfig.featuresRequired) {
            expectedConfig.featuresRequired = expectedConfig.featuresRequired.map((feature) => feature[1]);
         }
      ...
      beforeEach(() => {
         builder = new Builder(new Map(), buildOptions());
         builder.metaInfo = new Map();
         ...
         stubGetChangedFiles = jest.spyOn(Module.prototype, 'getChangedFiles').mockImplementation((revision) => {
            ...
            if (revision === 'rev2') {
               return {
                  changed: ['src/Module-theme/theme.json'],
                  deleted: []
               };
            }
         });
      ...
      test('should build modules list with ui module', async() => {
         ...
         builder.modules.set(module.id, module);
      ...
      test('should build modules list with unit module', async() => {
         const module = Module.buildModuleFromObject({
            ...moduleInfo,
            repository: {
               name: 'rep'
            },
      ...
      test('should build modules list with module has file of changed', async() => {
         const module = Module.buildModuleFromObject(moduleInfo);
         const moduleForMeta = Module.buildModuleFromObject({
            ...moduleInfo,
            revision: 'rev0'
         });

         builder.modules.set(module.id, module);
         builder.metaInfo.set(moduleForMeta.name, moduleForMeta);
      ...
      test('should build modules list with module has loadAfter construction', async() => {
         ...
         const module2 = Module.buildModuleFromObject(moduleInfo)

         builder.modules.set(module1.id, module1);
         builder.modules.set(module2.id, module2);
      ...
      test('should build modules list if one module with loadAfter construction is dependency another module', async() => {
         ...
         const module3 = Module.buildModuleFromObject(moduleInfo);

         builder.modules.set(module1.id, module1);
         builder.modules.set(module2.id, module2);
         builder.modules.set(module3.id, module3);
   ...
   describe('_saveBuildMeta()', () => {
      ...
      beforeEach(() => {
         ...
         const moduleForMeta1 = Module.buildModuleFromObject({
            type: 'ui',
            name: 'Module1',
            path: 'dir/Module1',
            id: '1',
         ...
         const moduleForMeta2 = Module.buildModuleFromObject({
            type: 'ui',
            name: 'Module2',
            path: 'dir/Module2',
            id: '2',
         ...
         const moduleForMeta3 = Module.buildModuleFromObject({
            type: 'ui',
            name: 'Module3',
            path: 'dir/Module3',
            id: '3',
         ...
         builder.metaInfo.set(moduleForMeta1.name, moduleForMeta1);
         builder.metaInfo.set(moduleForMeta2.name, moduleForMeta2);
         builder.metaInfo.set(moduleForMeta3.name, moduleForMeta3);
         builder.modules.set(module1.id, module1);
         builder.modules.set(module2.id, module2);
      ...
      test('should skip saving meta info if build result does not exists', async() => {
         ...
         expect(builder.metaInfo.size).toStrictEqual(3);
      ...
      test('should skip saving meta info if cache was dropped', async() => {
         ...
         expect(builder.metaInfo.size).toStrictEqual(2);
         expect(stubOutputFile.mock.calls.length).toStrictEqual(2);
   ...
   describe('_readBuildMeta()', () => {
      ...
      test('should create new meta info if file does not exists', async() => {
         ...
         expect(builder.metaInfo.size).toStrictEqual(0);
      });
      ...
      test('should read meta info if it is exists', async() => {
         ...
         expect(builder.metaInfo).not.toBeUndefined();
         expect(builder.metaInfo.size).toStrictEqual(1);
   ...
   describe('_linkCDNModules()', () => {
      ...
      beforeEach(() => {
         ...
         builder = new Builder(modules, buildOptions());
   ...
   describe('_tslibInstall()', () => {
      ...
      beforeEach(() => {
         ...
         modules.set('0df0fce3-53ba-47f8-987d-edf1fd078295', {
            type: 'ui',
            name: 'WS.Core',
            path: 'src/WS.Core',
            id: '0df0fce3-53ba-47f8-987d-edf1fd078295'
         });
         builder = new Builder(modules, buildOptions());
      });

      test('should not create symlink to tslib.js if module WS.Core is not exists', async() => {
         builder.modules = new Map();
   ...
   describe('startWatcher()', () => {
      let builder;
      let createStdoutHandlerFn;
      let createStdoutHandlerMock;

      beforeEach(() => {
         builder = new Builder(new Map(), buildOptions());
         NodeJS.mockClear();

         createStdoutHandlerFn = jest.fn();
         createStdoutHandlerMock = jest.spyOn(Builder, 'createStdoutHandler').mockImplementation(() => createStdoutHandlerFn);
      });

      afterEach(() => {
         createStdoutHandlerMock.mockRestore();
      });

      test('should create subprocess', async() => {
         await builder.startWatcher();

         expect(NodeJS).toHaveBeenCalledWith({
            type: 'fork',
            ...
            onData: createStdoutHandlerFn,
            onError: createStdoutHandlerFn,
            onMessage: undefined
         });
      });
   ...
   describe('prepareHotReload()', () => {
      test('should create options for hotReload', async() => {
         const modules = new Map();
         modules.set('db97ce92-b7ce-4ff3-9649-dc43fd3c36bf', {
            type: 'ui',
            name: 'HotReload',
            path: 'dir/HotReload',
            id: 'db97ce92-b7ce-4ff3-9649-dc43fd3c36bf'
         });

         const builder = new Builder(modules, buildOptions({
            hotReload: true
         }));
   ...
   describe('saveConfig()', () => {
      ...
      test('should save builder config to file', async() => {
         const testedConfig = (new Builder(new Map(), buildOptions())).config;
   ...
   describe('build()', () => {
      ...
      let stubLinkPythonModules;
      let stubCreateConfigRegionPlaceholder;
      ...
      beforeEach(() => {
         ...
         jest.spyOn(builder, '_buildRouterMap').mockImplementation(() => Promise.resolve());
         stubLinkPythonModules = jest.spyOn(builder, '_linkPythonModules').mockImplementation(() => Promise.resolve());
         stubSaveBuildMeta = jest.spyOn(builder, '_saveBuildMeta').mockImplementation(() => Promise.resolve());
         stubCreateConfigRegionPlaceholder = jest.spyOn(builder, '_createConfigRegionPlaceholder').mockImplementation(() => Promise.resolve());
      });
      ...
      afterEach(() => {
         ...
         stubLinkPythonModules.mockRestore();
         stubCreateConfigRegionPlaceholder.mockRestore();
      });
      ...
      test('should before prepare builder config and hotReload and start build and after save metainfo', async() => {
         ...
         expect(stubCreateConfigRegionPlaceholder.mock.calls.length).toStrictEqual(1);
      });

      test('should save metainfo if build fail', async() => {
      ...
      test('should save metainfo and create symlinks to cdn if build fail with options force', async() => {
         stubStartBuild.mockImplementation(() => Promise.reject('Build fail'));
         builder.options.set('force', true);
