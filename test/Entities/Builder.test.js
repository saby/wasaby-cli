...
describe('Builder', () => {
   ...
   describe('_tslibInstall()', () => {
      ...
      beforeEach(() => {
         ...
         stubExecute = jest.spyOn(builder.cmd, 'execute').mockImplementation(() => Promise.resolve());
      });
   ...
   describe('startWatcher()', () => {
      ...
      beforeEach(() => {
         ...
         stubFork = jest.spyOn(CMD, 'fork').mockImplementation(() => stubSubprocess);
      });

      afterEach(() => {
         stubFork.mockRestore();
      });

      test('should create subprocess', async() => {
         await builder.startWatcher();

         expect(stubFork.mock.calls.length).toStrictEqual(1);
         expect(stubFork.mock.calls[0][1]).toEqual([
            'buildOnChangeWatcher',
            '--config=src/logs/builderConfig.json',
            `--gulpfile=${require.resolve('sbis3-builder/gulpfile.js')}`
         ]);
         expect(stubFork.mock.calls[0][2]).toEqual({
      ...
      test('should create subprocess and subscribe to message event', async() => {
         ...
         expect(stubSubprocess.on.mock.calls.length).toStrictEqual(1);
         expect(stubSubprocess.disconnect.mock.calls.length).toStrictEqual(0);
         expect(stubSubprocess.on.mock.calls[0][0]).toStrictEqual('message');
         expect(stubSubprocess.on.mock.calls[0][1]).toStrictEqual(callback);
      });

      test('should create subprocess and disable IPC channel', async() => {
         await builder.startWatcher();

         expect(stubSubprocess.on.mock.calls.length).toStrictEqual(0);
         expect(stubSubprocess.disconnect.mock.calls.length).toStrictEqual(1);
      });
   });
   ...
   describe('startHotReload()', () => {
      ...
      beforeEach(() => {
         ...
         stubCMDExecute = jest.spyOn(builder.cmd, 'execute').mockImplementation(() => Promise.resolve());
      });
      ...
      test('should start hotReload server', async() => {
         ...
         expect(stubCMDExecute.mock.calls[0][0]).toEqual({
            file: 'build/application/HotReload/eventStream/third-party/server',
            args: {
               port: 6666
            }
         });
   ...
   describe('startBuild()', () => {
      ...
      test('should start build', async() => {
         ...
         expect(stubCMDExecute.mock.calls.length).toStrictEqual(1);
         expect(stubCMDExecute.mock.calls[0][0]).toEqual({
            file: require.resolve('gulp/bin/gulp.js'),
            command: 'build',
            args: {
               config: 'src/logs/builderConfig.json',
               gulpfile: require.resolve('sbis3-builder/gulpfile.js'),
