...
describe('Jinnee', () => {
   ...
   describe('_prepareService()', () => {
      ...
      beforeEach(() => {
         ...
         stubFsReadFile = jest.spyOn(fs, 'readFile').mockReturnValue(Promise.resolve(''));
         stubFsOutPutFile = jest.spyOn(fs, 'outputFile').mockReturnValue(Promise.resolve());
      });

      afterEach(() => {
         stubFsReadFile.mockRestore();
         stubFsOutPutFile.mockRestore();
      });
