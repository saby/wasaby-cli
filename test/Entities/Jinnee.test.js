...
describe('Jinnee', () => {
   ...
   describe('constructor()', () => {
      ...
      test('should create jinnee with default config', () => {
         const jinnee = new Jinnee(new Map(), buildOptions(), tsconfigPath, [], {}, 'iAmService');
         ...
         expect(jinnee.sbisProject).toStrictEqual('iAmService');
      ...
      test('should return exception if Jinnee path does not exists', () => {
         ...
         try {
            new Jinnee(new Map(), buildOptions(), tsconfigPath, [], {}, 'iAmService');
         } catch (err) {
      ...
      test('should detect Jinnee path if options pathToJinnee not exists', () => {
         new Jinnee(new Map(), buildOptions({
   ...
   describe('_detectPathToJinnee()', () => {
      ...
      beforeEach(() => {
         jinnee = new Jinnee(new Map(), buildOptions(), tsconfigPath, [], {}, 'iAmService', 'src/SDK');
      });
   ...
   describe('_prepareDeployCfg()', () => {
      ...
      beforeEach(() => {
         jinnee = new Jinnee(new Map(), buildOptions(), tsconfigPath, [], {}, 'iAmService');
      ...
      test('should build config stand for windows system', async() => {
         ...
         expect(stubWriteXmlFile.mock.calls[0][0]).toStrictEqual('src/logs/s3srv/InTest.s3deploy');
   ...
   describe('_prepareJinnee()', () => {
      ...
      test('should unpack jinnee if it is pack', async() => {
         ...
         expect(ChildProcess.mock.calls[0][0]).toEqual({
            commandLine: '7za x src/SDK/jinnee -y -obuild/jinnee > /dev/null && chmod -R 0777 build/jinnee',
            procOptions: {
               cwd: process.cwd()
            }
         });
   ...
   describe('start()', () => {
      ...
      beforeEach(() => {
         jinnee = new Jinnee(new Map(), buildOptions(), tsconfigPath, [], {}, {
            path: 'src/jinnee/JinneeTest.s3cld'
         });
         ChildProcess.mockClear();
      ...
      test('should start jinnee for build stand', async() => {
         ...
         expect(ChildProcess.mock.calls[0][0]).toEqual({
            env: 'src/jinnee',
            options: {
               project: 'src/jinnee/JinneeTest.s3cld',
   ...
   describe('build()', () => {
      ...
      beforeEach(() => {
         ...
         jest.spyOn(jinnee, '_buildRouterMap').mockImplementation(() => Promise.resolve());
         stubLinkPythonModules = jest.spyOn(jinnee, '_linkPythonModules').mockImplementation(() => Promise.resolve());
         stubSaveBuildMeta = jest.spyOn(jinnee, '_saveBuildMeta').mockImplementation(() => Promise.resolve());
         stubCreateConfigRegionPlaceholder = jest.spyOn(jinnee, '_createConfigRegionPlaceholder').mockImplementation(() => Promise.resolve());
      });
