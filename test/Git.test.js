...
describe('Git', () => {
   ...
   describe('merge()', () => {
      ...
      test('should execute merge for revision', async() => {
         const revision = 'a87c1079a382fc8a86904c36e0da7a420207fa5c';
      ...
      test('should execute abort merge and return exception if could not merge branches', async() => {
         ...
         try {
            ...
            expect(err.message).toStrictEqual(`Conflict in repository ${REPOS_URL} when merging current branch with 'remotes/origin/${branch}': Fail merge`);
         }
      });
   });
   ...
   describe('getNameFromUrl()', () => {
      test('should return name for https url', () => {
         ...
         expect(Git.getNameFromUrl('https://git/group/rep/')).toStrictEqual('group_rep');
         expect(Git.getNameFromUrl('git+https://git/group/rep/')).toStrictEqual('group_rep');
      });

      test('should return name for git url', () => {
         expect(Git.getNameFromUrl('git@git:group/rep.git')).toStrictEqual('group_rep');
         expect(Git.getNameFromUrl('git+shh://git@git:group/rep.git')).toStrictEqual('group_rep');
         expect(Git.getNameFromUrl('git://git/group/rep/')).toStrictEqual('group_rep');
      });
   });
