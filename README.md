...
1. Аргументом командной строки через --
2. Описать отдельным объектом в json файле. 
   * По умолчанию ищется файл wasaby-cli.config.json в папке откуда запущена команда.
   * Передать путь до файла него через параметр --config.
...
* ***localization {String[]}*** - Список кодов доступных языков. Первый язык из списка будет дефолтным. Пример кода для английского: en.
...
#### Параметры:
* ***ide {String}*** - Параметр запуска тестов на Jest из IDE. Принимаемое значение: "WebStorm, vscode".
...
* ***watcher {Boolean}*** - Запускает вотчер для ресурсов стенда, чтобы не перезапускать стенд, после каждной правки. Если запущен ватчер после команды buildProject, он будет выключен.
...
* [Prettier 3.0.3](https://prettier.io/)
#### Параметры:
* ***hookNames {String[]}*** - Имена хуков, которые надо инсталировать Доступны pre-commit и pre-push. По умолчанию инсталируется только pre-commit.
---

### buildAutoDoc
Команда соберёт автодокументацию по модулям репозитории, для отображения развернёт интерфейс, как на wi.sbis.ru.
#### Параметры:
* ***--rootReps <urls...>*** - Url-ы репозиториев, по всем модулям из них будет собрана документация.
* ***--modules <moduleNames...>*** - Имена модулей, по которым строить документацию.
* ***--dry*** - Выполнить только валидацию данных для документации без записи результатов на диск.
* ***--logLevel <value>*** - Минимальный уровень вывода диагностических сообщений в консоль. Доступны уровни: critical, error, warning.
* ***--cpuLimit <number>*** - Максимальное количество ядер процессора, которое разрешено использовать инструментом.
* ***--colors [boolean]*** - Управление цветом текста диагностических сообщений, выводимых в консоль.
* ***--noStandRebuild*** - Не выполнять пересборку стенда, если он уже собран.
* ***--noKaizenLoad*** - Не выполнять загрузку дерева участков Кайдзен. С этой опцией также отключается проверка существования участков Кайдзен.
* ***--force*** - Игнорировать ошибки сборки и не прерывать дальнейшие команды. Мы настоятельно не рекомендуем использовать данный флаг, игнорирование ошибок сборки может привести к ошибкам в дальнейших командах.
* ***--reporter <format>*** - Выполнить генерацию отчета о выполнении команды в указанный формат. Доступны форматы: text, xml, json.
* ***--port <number>*** - Запустить локальный стенд автодокументации на указанном порту. Если порт будет занят сгенерируется другой.
* ***--packageManager <name>*** - Использовать определенный пакетный менеджер для установки зависимостей. Доступны: npm, yarn. По умолчанию: npm.
---
...
* ***preCommitHooks {String[]}*** - Список обработчиков, которые надо запустить. Встроенные обработчики:
  ...
  * ***AmbiguousNames*** - Запускается по файлам с расширениями .ts, .js, .tsx, .jsx, .css, .less, .wml.

---

### runPrePushHook
Запускает обработчик для pre-push хука.
Подсистемы:
#### Параметры:
* ***tasks {String[]}*** - Список задач, которые надо выполнить в хуке:
  * ***units*** - Запускается команду [runUnitTests](https://git.sbis.ru/saby/wasaby-cli#runUnitTests). Для Jest запускаются юниты только по изменённым файлам, через опцию [findRelatedTests](https://jestjs.io/docs/cli#--findrelatedtests-spaceseparatedlistofsourcefiles).
  * ***load*** - Запускается команду [loadProject](https://git.sbis.ru/saby/wasaby-cli#loadProject).
  * ***build*** - Запускается команду [buildProject](https://git.sbis.ru/saby/wasaby-cli#buildProject).
...
### createModule
Создаёт новый модуль. Довыкачивает зависимости указаные в модуле.
#### Параметры:
...
* ***description {String}*** - Описания модуля.
* ***depends {String}*** - ID модулей от которых зависит модуль. Зависимости будут довыкачены.
* ***package {String}*** - Имя пакета к которому относиться модуль.
* ***enableAutodoc {Boolean}*** - Включить сборку автодоки для модуля. Не работает для модулей с тестами и демками.
* ***type {String}*** - Тип модуля. Варианты:
  * ***ui*** - Интерфейсный модуль. Значение по умолчанию.
  * ***test*** - Модуль c тестами.
  * ***demo*** - Модуль c демо примерами.
* ***environment {String}*** - В каком окружении запускать тесты.
  ...
  * ***JSDON*** - Тесты запускаются в окружении JSDOM(Фреймворк Jest).
...
если нужно обновить, какой либо репозитоий на специфическую ветку или для того что бы добавить репозитории в сборку
нужно в секции wasaby-cli вашего package.json файла, добавить параметр repositories, 
можно добавлять как ссылку на гит так и путь до своего локального репозитория, так же можно 
...
Для запуска тестов из интерфейса VS Code установите расширение [Jest](https://marketplace.visualstudio.com/items?itemName=Orta.vscode-jest) (Orta.vscode-jest).

1. В файле `.vscode/settings.json` проекта укажите команду запуска в режиме `--ide=vscode`:

   ```json
   {
       "jest.jestCommandLine": "node node_modules/wasaby-cli/cli.js runUnitTests --ide=vscode --NodeJS --report=console --consoleLevel=error",
       "jest.useJest30": true,
       "jest.runMode": "on-demand"
   }
   ```

   * `--ide=vscode` — режим интеграции с vscode-jest (аналог `--ide=WebStorm`);
   * `--report=console` — обязателен, чтобы wasaby-cli не ждал xunit-отчёт (результаты обрабатывает само расширение);
   * `--NodeJS` / `--JSDOM` — окружение, в котором запускаются тесты;
   * `--consoleLevel=error` — уменьшает шум в выводе, чтобы расширение корректно разбирало результаты.

2. Для отладки тестов добавьте в `.vscode/launch.json` конфигурацию:
   ...
   ```json
   {
       ...
       "request": "launch",
       "program": "${workspaceFolder}/node_modules/wasaby-cli/cli.js",
       "args": [
           "runUnitTests", "--ide=vscode", "--NodeJS", "--report=console", "--consoleLevel=error",
...
Тесты запускаются из панели Test Explorer или по кнопке рядом с тестом в файле.
...
Требование к json простое, в нём должен лежать объект, который попадёт в поле result в ответном запросе.
...
## Повышение важности правил линтеров
Если вы хотите повысить важность правила с ошибки до предупреждения, то нужно в package.json добавить конструкцию следующего вида:
  ```json
{
  "wasaby-cli": {
    "ESLint": {
...
Данный механизм можно использовать только для повышения важности правила, отключить правило таким образом не получится. Эти изменения повлияют только на работу локального линтера, т.е. подсветку в IDE и pre-commit хук.

Также, данный механизм можно использовать для повышения важности правил в отдельных папках:
  ```json
{
  "wasaby-cli": {
    "ESLint": {
