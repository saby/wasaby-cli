...
const ERROR_CODE = 2;
const INFO_MESSAGE = 'See the list of commands in the readme. https://git.sbis.ru/saby/wasaby-cli/#%D0%BA%D0%BE%D0%BC%D0%B0%D0%BD%D0%B4%D1%8B';
...
class Cli {
   ...
   async startScript(name, project) {
      ...
      try {
         ...
         const originalErr = err instanceof Error ? `${err.message}\n${err.stack}` : err;
   ...
   async run() {
      await logger.open({
         ...
         enableLabels: !!process.env.WASABY_CLI_ENABLE_LABELS || !this.config.params.get('isLocaleProject'),
         console: {
