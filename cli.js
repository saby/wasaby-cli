...
const cliCommand = new Command()
   ...
   .addOption(new Option('-c, --config <path>', 'Путь до до json файла конфигурации.')
      .default(pathUtils.join(process.cwd(), 'wasaby-cli.config.json'))
      .argParser(Config.pathParser)
   )
   .addOption(new Option('--store <path>', 'Путь до папки с репозиториями.')
      .argParser(Config.pathParser)
   )
   .addOption(new Option('--onlyChanges', 'Обрабатывать только изменённые файлы')
      .default(false)
   )
   .addOption(new Option('--repositories <urls...>', 'Список репозиториев, которые надо добавить в хранилище.')
      .default([])
      .hideHelp()
   )
...
process.on('exit', (code) => {
   logger.info(`Wasaby-cli exit code: ${code}`);
   logger.close();
});
