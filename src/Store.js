...
class Store {
   ...
   getDependentModules(depends, type = 'all', checkRepository) {
      ...
      for (const module of this.modules[type].values()) {
         for (const depend of depends) {
            const isDepend = Object.values(module.depends).includes(depend.name);
            const isOneRepository = module.repository === depend.repository || !checkRepository;
            const isATFTest = type === 'test' && module.framework === 'ATF' && module.environment === 'Python';

            if (isDepend && (isOneRepository || isATFTest)) {
               result.add(module);
            }
         }
      }
   ...
   getDependenciesModules(modules) {
      for (const module of modules) {
         for (const depend of this.getModules(Object.values(module.depends))) {
