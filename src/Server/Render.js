...
class Render {
   ...
   render(req, res) {
      ...
      const onSuccessHandler = (html) => {
         ...
         process.domain.req = undefined;
         process.domain.res = undefined;
      ...
      this.serverRouting.getPageSource(
         ...
         // Исключаем повторный толкание очереди. Если уже вызывался обработчик для страницы, то игнорируем then.
         // Если же обработчики не вызывались, значит случилась ошибка,
         // котору рендер не посчитал достойной, чтобы свалиться в catch и надо толкнуть очередь.
         if (pageGone === false) {
            logger.debug(`Page not gone but handler "then" was called. URL: ${req.url}`);
   ...
   async clearCache() {
      // Заканчиваем обработку всех текущих запросов и очишаем кеш
      this.queue.unshift(async() => {
         logger.debug('Clear cache start');

         const checkPath = nodePath.normalize(this.staticDir);

         for (const path of Object.keys(require.cache)) {
            // В Types/PromiseAPIDeffered портят Promise.prototype свойствами configurable: false.
            // И повторая поптка исполнит этот же код приводит к ошибки.
            // Поэтому его нельзя выкидывать и кеша
            if (path.includes('PromiseAPIDeferred')) {
               continue;
            }

            // Выкидываем из кеша только статику.
         ...
         logger.debug('Clear cache finish');
