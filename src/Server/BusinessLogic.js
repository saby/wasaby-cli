...
class BusinessLogic {
   static init(server, staticDirectory, defaultBLRoot) {
      ...
      server.get(/\/[fF]eature\/Get\//, (request, response) => {
   ...
   static getDemoName(url) {
      ...
      if (url.includes('/FrameDemoStand/')) {
         return url.split('/FrameDemoStand/')[1];
      }

      if (url.includes('/DemoStand/')) {
         return url.split('/DemoStand/')[1];
      }
      ...
      return url.split('/_/')[0].replace(urlClearRegExp, '');
   }
   ...
   static getDemoInfo(pageUrl) {
      const fixPath = pageUrl.replaceAll('%2F', '/').split('?')[0];
   ...
   static executeMethod(name, params, serviceUrl, pageUrl) {
      const methodName = transliterate(name);
      const demoInfo = BusinessLogic.getDemoInfo(pageUrl);
      const [servicePath] = serviceUrl.split('/service/');
      const methodHandlerPath = pathUtils.join('BLHandlers', servicePath, `${methodName.replace('.', '/')}.js`);
      const methodResultPath = pathUtils.join('BLHandlers', servicePath, `${methodName.replace('.', '/')}.json`);

      for (const root of demoInfo) {
         const handlerPath = pathUtils.join(root, methodHandlerPath);

         if (fs.existsSync(handlerPath)) {
            return require(handlerPath).execute(params, pageUrl);
         }
   ...
   static async processGet(req, res) {
      try {
         const referer = req.headers['x-refererpage'] || req.headers.referer;

         if (!referer) {
            throw new Error(`Метод ${req.query.method} вызван не со страницы c демо примером`);
         }

         logger.debug(`Обрабатываю БЛ запрос на сервис ${req.url} метода ${req.query.method} со страницы ${referer}.`);
   ...
   static async process(req, res) {
      try {
         ...
         logger.debug(`Обрабатываю БЛ запрос на сервис ${req.url} метода ${req.body.method} со страницы ${referer}.`);
