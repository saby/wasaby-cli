...
class ChildProcess {
   ...
   prepareValue(value) {
      // Для процессов, создаваемых с помощью exec или с опцией shell, необходимо оборачивать аргументы в кавычки
      if (this.type === 'exec' || this.procOptions?.shell) {
   ...
   static async isProcessAlive(id) {
      if (!id) {
         return false;
      }

      const prcLists = await psList();

      for (const prc of prcLists) {
         if (prc.pid === id && (prc.name === 'node.exe' || prc.name === 'node' )) {
            return true;
         }
      }
