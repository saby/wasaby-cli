...
class ChildProcess {
   ...
   _createProcess() {
      ...
      switch (this.type) {
         case 'fork': {
            ...
            this.process = childProcess.fork(this.exeFile, args, opts);

            logger.debug(`Create child process of type fork: \nfile: ${this.exeFile},\n args ${args}, \n opts: ${JSON.stringify(this.opts)}`);
         ...
         case 'spawn': {
            ...
            logger.debug(`Create child process of type spawn: \n env: ${this.env},\n args ${argsProc}, \n opts: ${JSON.stringify(this.procOptions)}`);
         ...
         case 'exec': {
            ...
            logger.debug(`Create child process of type exec: \n args ${argsProc}, \n opts: ${JSON.stringify(this.procOptions)}`);
   ...
   _subscribeProcess() {
      ...
      const promise = new Promise((resolve, reject) => {
         // The 'close' event is emitted after a process has ended and the stdio streams
         // of a child process have been closed.
         ...
         this.process.on('close', (code, signal) => {
            ...
            if (signal === 'SIGTERM' || signal === 'SIGINT' || signal === 'SIGKILL') {
               logger.debug(`Exit code: ${code}, signal: ${signal}`, this.processName);
      ...
      for (const [name, value] of Object.entries(options)) {
         ...
         if (value) {
            ...
            if (Array.isArray(value)) {
               if (this.assignmentOperator === ' ') {
                  result.push(`${prefixOption}${name}`);
                  value.forEach((arrValue) => {
                     result.push(this.prepareValue(arrValue));
                  });
               } else {
                  value.forEach((arrValue) => {
                     result.push(`${prefixOption}${name}${this.assignmentOperator}${this.prepareValue(arrValue)}`);
                  });
               }
            ...
            if (this.assignmentOperator === ' ') {
               result.push(`${prefixOption}${name}`);
               result.push(this.prepareValue(value));
            } else {
               result.push(`${prefixOption}${name}${this.assignmentOperator}${this.prepareValue(value)}`);
            }
         }
      }
   ...
   prepareValue(value) {
      // Для процессов, которые создаются с помощью fork, не нужно оборачивать аргументы в кавычки,
      // если те содержат пробел. Массив аргументов (строк) уже обеспечивает правильную передачу аргументов.
      // TODO: для процессов, которые создаются с помощью spawn (без опции shell),
      //  тоже не нужно оборачивать аргументы в кавычки.
      if (this.type === 'fork') {
         return value;
      }
   ...
   static kill(id) {
      try {
         ...
         if (error.code === 'ESRCH') {
            return true;  // процесса нет
         }
         // Другие ошибки (например, EPERM — нет прав)
         throw error;
      }
   };

   static isProcessAlive(id) {
      if (!id) {
         return false;
      }
