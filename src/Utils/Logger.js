...
class Logger {
   ...
   debug(content, label) {
      ...
      if (this.consoleLevel.debug) {
         this._consoleWrite(message, 'log');
      }
   ...
   error(error, label) {
      ...
      this._consoleWrite(errorMessage, 'error');
   ...
   _consoleWrite(message, level) {
      try {
         console[level](message);
      } catch (err) {
         // Может возникнуть ситуация, когда процесс, который запустил wasaby-cli, завершился, а мы нет.
         // Это приводит к тому, что при попытке записать в консоль, мы ловим ошибку "EPIPE: broken pipe, write".
         // Так как поток output родительского процесса уже закрылся.
         // И мы сваливаемся в цикл, событие 'uncaughtException' -> console.error -> событие 'uncaughtException'.
         // Поэтому оборачиваем вызов console и если ловим ошибку, то пишем её в файл и идем дальше.
         this.writeInChannel(this.channels.debugLogFile, message);
         this.writeInChannel(this.channels.errorLogFile, message);
      }
   }
}
