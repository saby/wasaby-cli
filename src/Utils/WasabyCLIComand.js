...
const MMProject = require('../Entities/MMProject');
...
async function prepareRepositoriesForMM(config) {
...
class WasabyCLICommand extends Command {
    ...
    action(callback) {
        super.action(async (userOptions, command) => {
            ...
            const enableMM = config.params.get('enableModuleManager') || config.params.get('isLocaleProject');

            if (enableMM) {
                await prepareRepositoriesForMM(config);
            } else {
            ...
            const ProjectConstructor = enableMM ? MMProject : Project;
            const project = new ProjectConstructor(configProject);
