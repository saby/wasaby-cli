...
async function createRep(link, config, versionRequired) {
    ...
    if (!await fs.pathExists(link)) {
        throw new Error(`Invalid git URL or non-existent path was passed in option "repositories" or "rootReps". ${link}`)
    }

    if (await Repository.isGitRep(link)) {
...
async function prepareRepositories(config) {
    ...
    const wasabyCLIRep = new Directory(pathUtils.join(__dirname, '../..'));
...
async function getRCVersion(rootRep) {
    if (rootRep instanceof Repository) {
        const rcBranch = await rootRep.getCurrentRCBranch();

        return rcBranch ? rcBranch.replace('rc-', '') : '';
    }
}
...
class WasabyCLICommand extends Command {
    ...
    addOptions(options) {
        ...
        for (const option of options) {
            if (!addedOptions.has(option.flags)) {
                addedOptions.add(option.flags);
    ...
    action(callback) {
        super.action(async (userOptions, command) => {
            ...
            if (config.params.get('isLocaleProject')) {
                ...
                // Цвет принудительно включаем только при выводе в настоящий терминал:
                // иначе дочерние процессы (eslint и др.) красят вывод в пайп,
                // а часть консолей печатает остатки ESC-последовательностей как мусор.
                if (process.stdout.isTTY) {
                    process.env.FORCE_COLOR = true;
                }

                rootRep = await createRep(process.cwd(), config);

                if (!config.params.get('rc')) {
                    const rcVersion = await getRCVersion(rootRep) || config.params.get('npmProjectVersion');

                    config.params.set('rc', `rc-${rcVersion}`);
                }

                await updateGitinore(config);
            } else if (!config.params.get('rc')) {
                config.params.set('rc', config.params.get('cliVersion'));
            }
