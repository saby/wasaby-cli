...
const COMMIT_SHA = /^\w{40}$/;
const REG_EXP_CHECK_REMOTE = /\[remote "origin"]\s+url\s=\s[\w@.:/-]+\s+fetch/;
const REG_EXP_CHECK_GIT_URL = /^(git@|(https|(git(\+(ssh|https))?)):\/\/)[\w.@:/-]+(#|$)/;
...
class Git extends Directory {
    ...
    * @param [cfg.protocol] {String} Как скачивать репозитории по https или по ssh
    */
   constructor(cfg) {
      const path = cfg.path || pathUtils.join(cfg.dir, cfg.name || Git.getNameFromUrl(cfg.url));

      super(path);

      this.protocol = cfg.protocol || 'ssh';
      this.https = Git.convertUrlToHttps(cfg.url);
      this.ssh = Git.convertUrlToSSH(cfg.url);

      this.freezeStateOfCommits = !!cfg.freezeStateOfCommits;
   }

   get url() {
      return this[this.protocol];
   }

   async init() {
      await fs.mkdir(this.path);
      await this._execCommand('init');
   ...
   async remote(command, params, options) {
      await this._execCommand(
         `remote ${command}`,
         options,
         params
   ...
   async checkout(revision, options) {
      ...
      logger.debug(`Checkout to branch ${revision}`, this.name);
   ...
   async merge(branch, options) {
      ...
      try {
         ...
         const error =  new Error(`Conflict in repository ${this.url} when merging current branch with '${mergeWith}': ${e}`);

         error.gitType = 'mergeConflict'
   ...
   async catFile(revision, options) {
      await this._execCommand(
         'cat-file',
         options,
         [
   ...
   async config(name, accessor = 'get', value = '') {
      if (accessor === 'get') {
         const result = this._execCommand(
            'config',
            {
               get: true
            },
   ...
   _prepareConfigGit() {
      if (!this.configReady) {
         this.configReady = (async() => {
            ...
            await this.config('core.quotepath', 'set', false);
   ...
   static getNameFromUrl(url) {
      ...
      return new URL(name).pathname.slice(1).replaceAll('/', '_');
   }
   ...
   static convertUrlToHttps(url) {
      const normalizeUrl = url
         ...
         .replace(/^(git@)/, 'https://')
         .replace(/:(\w)/g, '/$1');
      const parsedUrl = new URL(normalizeUrl);

      return `https://${parsedUrl.hostname}${parsedUrl.pathname}`;
   }

   static convertUrlToSSH(url) {
      const normalizeUrl = url
         .replace(/(\.git|\/)$/, '')
         .replace(/^(git@)/, 'https://')
         .replace(/:(\w)/g, '/$1');
      ...
      return `git@${parsedUrl.hostname}:${parsedUrl.pathname.slice(1)}.git`;
   }

   static isGitUrl(url) {
      return REG_EXP_CHECK_GIT_URL.test(url);
   }

   static async isGitRep(path) {
      try {
         const proc = new GitProcess({
            command: 'rev-parse',
            options: {
         ...
         return result[0].trim() === 'true';
      } catch (err) {
         return false;
      }
   }

   static isCommitSHA(revision) {
      return COMMIT_SHA.test(revision);
   }

   static isBranch(branch) {
      return !COMMIT_SHA.test(branch);
   }
}
