...
class Config {
   ...
   readPackageJson() {
      ...
      result.set('npmProjectVersion', packageJson.version.split('.', 2).join('.'));
      ...
      if (packageJson.name !== 'wasaby-cli') {
         ...
         if (result.has('rootReps')) {
            result.get('rootReps').push(process.cwd());
         } else {
            result.set('rootReps', [process.cwd()]);
         }
      }
   ...
   _mergeConfigs(packageConfig, userConfig) {
      const result = new Map([...packageConfig, ...userConfig]);

      for (const [name, value] of result.entries()) {
         if (Array.isArray(value)) {
            const newArr = [
               ...new Set([
                  ...(packageConfig.get(name) || []),
                  ...(userConfig.get(name) || [])
               ])
            ];
