...
function extractFeaturesList(features, useCustomPath) {
   const result = [];
   ...
   for (const feature of features) {
      ...
      if (useCustomPath && feature.$.path) {
         currentFeature.path = feature.$.path;
      }

      result.push([currentFeature.name, currentFeature]);
   }
...
class Module {
    ...
    * @param cfg.description {String} Описание модуля.
    * @param cfg.depends {Object<name, id>} Массив имен модулей, от которых зависит данный модуль.
    ...
    * @param [cfg.enableAutodoc] {Boolean} Включена ли сборка автодоки для модуля.
    * @param [cfg.featuresProvided] {Map<String, Object>} Предоставляемые модулем фичи.
    * @param [cfg.featuresRequired] {Map<String, Object>} Необходимые фичи для модуля.
    * @param [cfg.typescript] {TypeScriptCfg} Объект с настройками TypeScript.
    * @param [cfg.autodoc] {boolean} Содержит ли модуль документацию в новом формате.
    */
   constructor(cfg) {
      this.type = 'module';
      ...
      this.path = cfg.path;
      this.dir = pathUtils.dirname(cfg.path);
      ...
      this.enableAutodoc = cfg.enableAutodoc;
      this.description = cfg.description || '';
      this.shortHash = cfg.shortHash;

      this.featuresProvided = cfg.featuresProvided && new Map(cfg.featuresProvided);
      this.featuresRequired = cfg.featuresRequired && new Map(cfg.featuresRequired);
   ...
   merge(module) {
      ...
      this.shortHash = module.shortHash;
   ...
   isMatchedFilter(filter) {
      for (const [name, values] of Object.entries(filter)) {
         if (name === 'repository') {
            if (!values.includes(this.repository.name)) {
               return false;
            }
         ...
         if (name === 'featuresProvided' && this.featuresProvided) {
            if (!this.featuresProvided) {
               return false;
            }

            if (!(values.some((val) => this.featuresProvided.has(val)))) {
               return false;
            }
   ...
   serialize() {
      return {
         ...
         forCDN: this.forCDN,
         featuresProvided: this.featuresProvided && [...this.featuresProvided],
         featuresRequired: this.featuresRequired && [...this.featuresRequired],
   ...
   buildAst() {
      const ast = {
         ui_module: {
            $: {
               name: this.name,
               id: this.id,
               kaizen_zone: this.kaizen.id,
               responsible: this.kaizen.responsible,
               responsible_uuid: this.kaizen.responsibleUuid,
            },
            description: [
               this.description
            ],
      ...
      for (const [id, name] of Object.entries(this.depends)) {
         ast.ui_module.depends[0].module.push({
            $: {
               name,
               id,
            }
      ...
      if (this.enableAutodoc) {
         ast.ui_module.$.autodoc = '1';
      }

      if (this.package) {
         ast.ui_module.$.package = this.package;
      }
         ...
         ast.ui_module.typescript = [
            {
               arg: [
                  {
                     $: {
                        typecheck: this.typescript.typecheck || "0"
                     }
                  }
               ]
            }
   ...
   static async buildModuleFromXml(path, defaultOptions) {
      ...
      if (module[type].depends && module[type].depends[0]) {
         ...
         if (depends.ui_module) {
            depends.ui_module.forEach((item) => {
               moduleInfo.depends[item.$.id] = item.$.name;
            });
         }
      ...
      if (module[type].features_provided?.[0]?.feature) {
         moduleInfo.featuresProvided = extractFeaturesList(module[type].features_provided[0].feature, true);
      }

      if (module[type].features_required?.[0]?.feature) {
         moduleInfo.featuresRequired = extractFeaturesList(module[type].features_required[0].feature, false);
      }
      ...
      if (Module.isDemo(moduleInfo.name)) {
   ...
   static isDemo(moduleName) {
      return moduleName.endsWith('-demo') || moduleName.endsWith('-Demo');
   }

   isSuperbundles() {
      return this.name.startsWith('Superbundles');
   }
}
