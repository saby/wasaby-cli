...
class Project {
   ...
   async build() {
      if (this.store.locked) {
         const data = this.store.lockInfo;

         throw new Error(`The repository store at ${this.store.path} is locked by the process ${data.whoLocked} running from the directory ${data.cwd}`);
      }

      await this.store.lock('buildProject');
    ...
    */
   async buildAutoDoc() {
      if (this.options.get('experimentalBuilder')) {
         return this.buildAutoDocExperimental();
      }

      await this.subsystemController.init('AutoDoc');

      const autoDoc = this.subsystemController.get('AutoDoc');
      const testedModules = this.getTestableModules();
      const rootRep = [...this.options.get('rootReps').values()][0];
      const dataVersion = (await rootRep.revParse('HEAD', {
         'abbrev-ref': true
      })).split('/')[0].replace('rc-', '');

      const { modules } = this.store.getDependencies(testedModules);

      await autoDoc.build(new Set([...modules.values()]), dataVersion);
   ...
   async buildAutoDocExperimental() {
      await this.subsystemController.init('AutoDocExperimental');

      const autoDoc = this.subsystemController.get('AutoDocExperimental');
      const testedModules = this.getTestableModules();
      const rootRep = [...this.options.get('rootReps').values()][0];
      const branchName = await rootRep.revParse('HEAD', {
         'abbrev-ref': true
      });
      const dataVersion = branchName.split('/').shift().replace('rc-', '');
