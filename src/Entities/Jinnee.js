...
class Jinnee extends Builder {
   constructor(modules, options, tsconfig, requiredFeature, routingModules, sbisProject, sdkPath) {
      super(modules, options, tsconfig, requiredFeature, routingModules);
   ...
   async build() {
      ...
      try {
         ...
         await Promise.all([
            ...
            this._buildRouterMap(this.servicePath),
         ]);
      } catch (err) {
         throw err;
      } finally {
   ...
   async _linkCDNModules() {
      ...
      const CDNDir = pathUtils.join(this.servicePath, 'cdn');

      if (!fs.pathExistsSync(CDNDir)) {
         await fs.ensureSymlink(pathUtils.join(this.config.output, 'cdn'), CDNDir);
      }
      ...
      for (const module of this.modules.values()) {
         if (module.forCDN) {
            promises.push((async() => {
               ...
               const target = pathUtils.join(this.servicePath, module.name);
   ...
   async _prepareService() {
      ...
      await this.sbisProject.addUIModules(this.buildModules);
