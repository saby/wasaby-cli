...
class Jinnee extends Builder {
   constructor(modules, options, tsconfig, service, sdkPath) {
   ...
   async _prepareService() {
      ...
      await pMap(services, async(service) => {
         ...
         service.path = pathUtils.join(logger.dir, 's3srv', service.name);
      }, {
         concurrency: 10
      });
      ...
      let content = await fs.readFile(this.s3cldPath, 'utf8');

      content = content.replace(
          /"[\w.]+\/InTestUI\.s3srv"/,
          `"${pathUtils.relative(pathUtils.dirname(this.s3cldPath), this.service.path)}"`
      );

      await fs.outputFile(this.s3cldPath, content);
   }
}
