...
class Project {
   ...
   async load() {
      ...
      // Грузим репозитории, которые были переданы в --rep и --repositories c указанием ветки.
   ...
   async build() {
      ...
      const { modules } = this.store.getDependencies(rootModules);
      ...
      this.builder = new Builder(
         // TODO Совместимость со старой архитектурой удалить, когда полностью перейдём на Module Manager
         this._convertToModulesMap(modules),
         this.options,
         tsconfig.path,
         PROVIDER_FEATURE,
   ...
   // TODO Совместимость со старой архитектурой удалить, когда полностью перейдём на Module Manager
   _convertToModulesMap(modules) {
      const result = new ModulesMap();
