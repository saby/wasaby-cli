...
class Builder {
   constructor(modules, options, tsconfig, requiredFeature, routingMap) {
      ...
      this.statusPath = pathUtils.join(this.options.get('resources'), 'status.json');
      this.requiredFeature = requiredFeature || [];
      this.routerMap = routingMap || {};

      if (typeof modules === 'string' && fs.existsSync(modules)) {
         const pathInfo = pathUtils.join(modules, 'status.json');

         if (fs.existsSync(pathInfo)) {
            const info = require(pathInfo);

            if (fs.existsSync(info.builderConfig)) {
               this.configPath = info.builderConfig;
               this.config = require(info.builderConfig);
      ...
      this.createConfig(tsconfig);
   }

   createConfig(tsconfig) {
   ...
   async watcher(onChangeCallback) {
      const promises = [];
      const watcherInfoPath = pathUtils.join(this.options.get('resources'), 'watcher.json');

      if (fs.existsSync(watcherInfoPath)) {
         const watcherInfo = require(watcherInfoPath);

         NodeJS.kill(watcherInfo.pid);
      }

      await fs.outputFile(watcherInfoPath, JSON.stringify({
         pid: process.pid,
      }));
   ...
   async _saveBuildMeta() {
      ...
      await Promise.all([
         fs.outputFile(this.statusPath, serializeStatus),
   ...
   async _createConfigRegionPlaceholder() {
      const result = {};
      const locConfigsModule = this.modules.get('42b71438-721f-4b7c-8dfa-443444d7a0e7');
      const I18nModule = this.modules.get('832cda0a-e19c-4ffc-a1d1-47edbb6b7501');
      const regions = new Set();
      const languages = new Set();
      const regionPath = pathUtils.join(locConfigsModule.path, 'localization_configs/region');
      const langPath = pathUtils.join(I18nModule.path, 'locales');

      for (const nameFile of (await fs.readdir(regionPath))) {
         regions.add(require(pathUtils.join(regionPath, nameFile)));
      }

      for (const nameFile of (await fs.readdir(langPath))) {
         if (nameFile.endsWith('.ts')) {
            languages.add(nameFile.split('.')[0]);
         }
      }

      for (const langCode of languages) {
         const dict = require(pathUtils.join(locConfigsModule.repository.path, 'www/service/Модули/i18n/lang', `${langCode}.json`));
         const langConfig = {};
