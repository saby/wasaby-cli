...
class Builder {
   ...
   async _convertThemes() {
      const promises = [];

      for (const module of this.modules.getModules()) {
         if (module.name.endsWith('-theme')) {
            const pathToJson = pathUtils.join(module.path, 'theme.json');

            if (fs.existsSync(pathToJson)) {
               promises.push((async () => {
                  const themeLess = new Less(pathToJson.replace('theme.json', '_new_theme.less'));
                  const pathToTheme = pathUtils.join(module.path, 'theme.less');

                  this.modifyFiles.add(themeLess);

                  await themeLess.fromJSON(pathToJson);
                  await themeLess.save();

                  if (fs.existsSync(pathToTheme)) {
                     const less = new Less(pathToTheme);

                     this.modifyFiles.add(less);

                     less.read();

                     if (!less.hasImport(themeLess.path)) {
                        less.addImport(themeLess.path);
                        await less.save();
                     }
                  }
               })());
   ...
   convertChangedFiles(files, modulePath) {
      ...
      for (const filePath of files) {
         ...
         if (filePath.endsWith('-theme/theme.json')) {
            result.push('./_new_theme.less');

            if (pathUtils.join(modulePath, 'theme.less')) {
               result.push('./theme.less');
            }
         }
      }
   ...
   convertDeletedFiles(files, modulePath) {
      const result = [];
   ...
   _resetModifyFile() {
      return pMap(this.modifyFiles, async(file) => {
         await file.reset();
      }, {
         concurrency: 10
      });
         ...
         if (this.metaInfo.has(module.name)) {
            ...
            if (revisions) {
               try {
                  ...
                  builderConfigModule.deletedFiles = this.convertDeletedFiles(changedFiles.deleted, module.path);
               } catch (error) {
