...
const Yarn = require('../Process/Yarn');
...
class AutoDoc {
   constructor(options, path) {
      ...
      const PackageManager = options.get('packageManager') === 'yarn' ? Yarn : Npm;
   ...
   async tryLoadKaizen() {
      ...
      const script = new NodeJS({
         ...
         options,
         params: ['kaizen', 'component']
      });

      logger.info(`Running load kaizen and component data for autodoc`);

      return script.run()
         .catch(() => {
            throw Object.defineProperty(
               new Error('Failed to load kaizen and component data'),
               'exitCode',
               {
                  value: script.exitCode
               }
            )
         });
   }
   ...
   async build(modules) {
      ...
      return script.run()
         .catch(() => {
            throw Object.defineProperty(
               new Error('Failed to build autodoc data'),
               'exitCode',
               {
                  value: script.exitCode
               }
   ...
   async startServer() {
      ...
      serverOptions.set('port', this.options.get('port'));
