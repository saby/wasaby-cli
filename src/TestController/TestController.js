...
function addWidthHeightFromViewBox(svgContent) {
   // Ищем viewBox="0 0 100 100"
   const viewBoxMatch = svgContent.match(/viewBox\s*=\s*["'](\d+\s+\d+\s+(\d+)\s+(\d+))["']/i);

   if (viewBoxMatch) {
      const width = viewBoxMatch[2];
...
function staticsHandler(req, res, root, url, defaultHandler) {
   if (url.endsWith('svg')) {
      const filePath = pathUtils.join(root, url);
      ...
      fs.readFile(filePath, 'utf8', (err, content) => {
         ...
         const modifiedContent = addWidthHeightFromViewBox(content);

         res.set('Content-Type', 'image/svg+xml');
         res.send(modifiedContent);
      });
   } else {
      defaultHandler(res, url);
   }
}
...
class TestController {
   ...
   async runUnitTest() {
      ...
      if (Object.keys(jestExecutors).length > 0) {
         ...
         await this.runTests(jestExecutors, { staticServerPort: server.port });
