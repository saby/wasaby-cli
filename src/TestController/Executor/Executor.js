...
const whiteListModuleConsoleErrors = new Set(require('./WhiteListModuleConsoleErrors.json'));
...
function onError(error, result, errors) {
   ...
   if (/snapshots? obsolete/.test(err)) {
      errors.push(error);
...
class Executor {
   constructor(cfg) {
      ...
      if (this.isDisableConsoleFilter()) {
         this.onError = onError;
      }
   }

   isDisableConsoleFilter() {
      if (this.options.get('disableConsoleFilter')) {
         return false;
      }

      for (const module of this.modules) {
         if (whiteListModuleConsoleErrors.has(module.name)) {
            return true;
         }
      }
