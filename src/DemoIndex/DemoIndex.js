...
class DemoIndex {
   constructor(cfg) {
      ...
      this.demoModules = this._getDemoModulesList(cfg.modules);
   }
   ...
   create() {
      try {
         ...
         let htmlContents = DemoIndex.buildHTMLMetaInfo(this._getToMetaInfo());
   ...
   _getToMetaInfo() {
      const resourceStatus = pathUtils.join(this.options.get('resources'), 'status.json');

      if (fs.existsSync(resourceStatus)) {
         return require(resourceStatus);
      }

      const artifactsStatus = pathUtils.join(this.options.get('artifactsDir'), 'buildProject', 'status.json');

      if (fs.existsSync(artifactsStatus)) {
         return require(artifactsStatus);
      }
   }

   _getDemoModulesList(modules) {
      const result = new Map();
      const contentsPath = pathUtils.join(this.options.get('resources'), 'contents.json');
      const availableModules = fs.existsSync(contentsPath) ? fs.readJSONSync(contentsPath).modules : {};

      for (const module of modules.values()) {
         if (module.type === 'demo' && availableModules.hasOwnProperty(module.name)) {
            result.set(module.name, pathUtils.join(this.options.get('resources'), module.name));
         }
      }
   ...
   _buildIndex(htmlContents) {
      ...
      fs.outputFileSync(
         pathUtils.join(this.options.get('resources'), 'index.html'),
         Template.page(title, htmlContents)
      );
   }

   static buildHTMLMetaInfo(metaInfo) {
      if (!metaInfo) {
         return '';
      }

      const date = new Date(metaInfo.date);
      const strDate = `${date.getDate()}.${date.getMonth() + 1}.${date.getFullYear()} ${date.getHours()}:${date.getMinutes()}`;
      const status = metaInfo.stable ? 'stable' : 'unstable';

      return `<div class="header"><h1>Version: ${metaInfo.version} || BuildDate: ${strDate} || Status: ${status}</h1></div>`;
   }
