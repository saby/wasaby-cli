...
class ModuleManager {
    ...
    static async getDepsList(version, modules) {
        ...
        count++;
        await logger.writeFile(`getDepsList_Response_BL_${count}.json`, JSON.stringify(data, null, 3));
    ...
    static async getDependedModules(version, modules, repositories) {
        ...
        count++;
        await logger.writeFile(`getDependedModules_Response_BL_${count}.json`, JSON.stringify(data, null, 3));

        return data.result || [];
    }

    /**
     * Получить короткие хеши для модулей
     * @param ids {String} id модулей
     * @returns {Promise<Record<String, String>>}
     */
    static async getShortHashForModules(ids) {
        const data = JSON.parse(await ModuleManager.BlGetModulesHashes(ids));

        await logger.writeFile(`getShortHashForModules_Response_BL.json`, JSON.stringify(data, null, 3));

        return data.result || {};
    }

    /**
     * БЛ запрос за короткими хеши для модулей
     * @param ids {String} id модулей
     * @returns Promise<String>
     */
    static async BlGetModulesHashes(ids) {
        const content = Buffer.from(JSON.stringify({
            'jsonrpc': '2.0',
            'protocol': 7,
            'method': 'Metadata.GetModulesHashes',
            'params': {
                ModulesIds: ids
            },
            'id': 1
        ...
        const body = {
            ...
            headers: {
                ...
                'x-calledmethod': 'Metadata.GetModulesHashes',
                'x-originalmethodname': 'TWV0YWRhdGEuR2V0TW9kdWxlc0hhc2hlcw==',
