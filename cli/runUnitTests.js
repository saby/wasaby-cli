...
const options = [
   ...
   new Option(
      ...
      'Параметр для Jest. Задает список имен репортеров покрытия. Доступные репортеры: clover, json, lcov, text, html. Пример: --coverageReporters json html'
   ),
   new Option(
      '--coverageDirectory <string>',
      'Параметр для Jest. Задает полный путь до директории, куда Jest запишет отчет о покрытии'
   ),
   new Option(
   ...
   // Опции, которые расширение vscode-jest (Orta.vscode-jest) добавляет к команде
   // при запуске теста из Test Explorer / code lens. Без объявления commander
   // считает их неизвестными и завершает команду с ошибкой.
   new Option('--testLocationInResults', 'vscode-jest')
       .hideHelp(),
   new Option('--json', 'vscode-jest')
       .hideHelp(),
   new Option('--useStderr', 'vscode-jest')
       .hideHelp(),
   new Option('--outputFile <path>', 'vscode-jest')
       .hideHelp(),
   new Option('--listTests', 'vscode-jest')
       .hideHelp(),
   new Option('--watchAll', 'vscode-jest')
       .hideHelp(),
   new Option('--no-coverage', 'vscode-jest')
       .hideHelp(),
   new Option('--findRelatedTests <file>', 'vscode-jest')
       .hideHelp(),
   new Option(
