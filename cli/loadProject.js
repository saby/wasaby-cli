...
const options = [
    ...
    new Option(
       '--loadFullHistory',
       'Грузить ли полную историю репозиториев.'
    )
       .default(false),
    new Option(
       '--restrictionsForStore <path>',
       'Путь до файла, с описанием ограничений для хранилища.'
    )
       .argParser(Config.pathParser),
    new Option('--rootReps <urls...>', 'Url-ы репозиториев. Их модули, будут считаться корневыми в проекте.')
       .default([]),
    new Option(
        '--modules <moduleNames...>',
   ...
   new Option(
      ...
      'Отключить дозагрузку модулей через Module Manager.'
   )
      .default(false)
      .hideHelp(),
];
