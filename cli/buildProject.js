...
const options = [
   ...
   new Option(
       ...
       .argParser(Config.pathParser),
   new Option('--rootReps <urls...>', 'Url-ы репозиториев, все модули из них будет собраны.')
      .default([]),
   new Option(
       '--modules <moduleNames...>',
   ...
   new Option(
       '--pythonTestDir <path>',
       'Путь до папки с тестовыми Python модулями.'
   )
       .argParser(Config.pathParser)
       .hideHelp(),
   new Option(
   ...
   new Option(
      '--disableMetatypes',
      'Отключить генерацию метатипов'
   )
      .hideHelp(),
   new Option(
      '--disableFontsGenerate',
      'Отключить генерацию шрифтов'
   )
      .hideHelp(),
   new Option(
       '--debug',
   ...
   new Option(
      '--reactVersion <number>',
      'Версия React-а, который надо использовать.'
   )
      .choices(['17', '19'])
      .default('17')
      .hideHelp(),
